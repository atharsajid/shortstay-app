...
class BookingRepository {
  static Future<List?> loadBookingForm(id) async {
    ...
    if (response.success) {
      ...
      return [bookingStyle, BookingPaymentModel.fromJson(response.data['payment'])];
    } else {
      ///Notify
  ...
  static Future<List?> loadList({
    ...
    Map<String, dynamic> params = {
      "user_id": AppBloc.userCubit.state?.id,
    };
    if (sort != null) {
    ...
    final ResultApiModel response = await Api.requestBookingList(params);
    if (response.success) {
